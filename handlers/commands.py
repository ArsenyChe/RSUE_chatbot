...
async def cmd_show_questions(message: Message, command: CommandObject):
    ...
    if len(message.text.split()) == command_len:
        ...
        try:
            ...
            await database_admin_log.add_log(message.from_user.username, message.text, datetime.now())
...
async def cmd_show_patterns(message: Message, command: CommandObject):
    ...
    if len(message.text.split()) == command_len:
        ...
        print(message.from_user.username, command.args, datetime.now())
        await database_admin_log.add_log(message.from_user.username, message.text, datetime.now())
...
async def cmd_nn_training(message: Message, command: CommandObject):
    command_len = 3
    if len(message.text.split()) == command_len:
        train_batch_size, train_epochs = command.args.split()
        nn = NeuralNetworkTraining("data.json")
        nn.read_data()
        X_train, Y_train = nn.preprocess_data()
        nn.build_model()
        info = nn.train_model(X_train, Y_train, int(train_batch_size), int(train_epochs))
        await message.answer(f"Точность: {info.history['accuracy']}")
        await message.answer(f"Потери: {info.history['accuracy']}")
        await message.answer(f"Нейросеть переобучилась")
        await database_admin_log.add_log(message.from_user.username, message.text, datetime.now())
    else: await message.answer('Команда написана неправильно. /trainingnn <обучающая выборка> <количество эпох> Переобучает нейросеть')
...
async def cmd_edit_intent(message: Message, command: CommandObject):
    ...
    if len(message.text.split()) == command_len:
        ...
        await database_admin_log.add_log(message.from_user.username, message.text, datetime.now())
    else: await message.answer('/editintent <тема> <новое наименование темы> Изменяет тему  с пустыми шаблонами вопросов и ответами на вопросы')
